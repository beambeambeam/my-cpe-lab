...
int main() {
  ...
  while (1) {
    ...
    if (num_array_student >= num_students) {
      printf("Cannot add %s into the student list. The number of student "
             "cannot exceed %d, Please type 'END' to sort student score and "
             "end the program.\n",
             ans_name, num_students);
    } else {
      printf("Enter %s's Score: ", ans_name);
      scanf("%d", &ans_score);

      strcpy(name[num_array_student], ans_name);
      score[num_array_student] = ans_score;
